...
proposal = c.deepcopy(warehouse)

for index in range(len(proposal)):
    if proposal[index].get('weight') > 300:
        # print(proposal[index])
        v = proposal[index]['price']
        proposal[index]['price'] = v - (v * 20 / 100)
        # print(proposal[index]['price'])
